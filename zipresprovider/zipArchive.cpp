...
CZipFile::CZipFile()
	...
	BOOL CZipArchive::Open(LPCTSTR pszFileName, HMODULE hModule /*= NULL*/)
	{
		Close();

		HANDLE hFile = OpenFile(pszFileName, hModule);
	...
	HANDLE CZipArchive::OpenFile(LPCTSTR pszFileName, HMODULE hModule)
	{
		...
		{
			HRSRC hResInfo = ::FindResource(hModule, pszFileName, _T("ZIP"));
			if (hResInfo == NULL)
				return INVALID_HANDLE_VALUE;
